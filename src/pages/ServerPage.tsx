    ...
    >
      ...
      <Panel era="final" title={<SectionHeading id="sim">バランスを測れるようにする</SectionHeading>} accent={accent}>
        ...
        <Disclosure summary="この方法でいちばん学んだこと" accent={accent}>
          ...
          <p className="mt-3">
            一度は<strong>測り直しを忘れたまま先に進んでしまい</strong>、あとから「正確に打つ人が有利すぎる状態」で動いていたことに気づいています。以降は「お題や難易度に触ったら必ず測り直す」を手順として残しました。
          </p>
          <p className="mt-3">
      ...
      <Panel era="final" title={<SectionHeading id="spectator">脱落したあとの時間</SectionHeading>} accent={accent}>
        ...
        <Disclosure summary="当初は脱落した瞬間に情報が止まっていた" accent={accent}>
          <p>
            もともと秒読みは<strong>まだ生きている店にだけ</strong>配っていました。脱落した瞬間から届かなくなるので、秒数が最後の値で固まったまま、順位表だけが動き続ける画面になります。
          </p>
          <p className="mt-3">
            これはクライアント側から<strong>「順位は動いているのに秒読みが止まっている」</strong>という指摘をもらって気づきました。サーバー側だけ見ていると、送っていないことに気づけない類の抜けです。
          </p>
          <p className="mt-3">
            直し方としては、通信の形は変えずに<strong>配る相手を広げるだけ</strong>にしました。クライアント側は受け取る用意がすでにあったので、あちらの修正は不要でした。増える通信量も事前に見積もって、問題ない範囲であることを確かめてから入れています。
          </p>
          <p className="mt-3">
            なお最後の区切りでは、表示のうえでは<strong>1位以外が脱落する</strong>ように見せています。裏では全員を同じように処理していて、見せ方だけを変えている形です。
          </p>
        </Disclosure>
      </Panel>
